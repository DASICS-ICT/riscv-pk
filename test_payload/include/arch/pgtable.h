...
#define PGDIR_PA 0x81000000lu  // use bootblock's page as PGDIR
...
static inline void set_pkey(PTE *entry, uint64_t pkey)
{
    // printk("[DEBUG] *entry = 0x%lx\n", *entry);
    *entry &= ~((uint64_t)(0x1ful << 54));
    // printk("[DEBUG] *entry = 0x%lx\n", *entry);
    *entry |= (pkey & 0x1ful) << 54;
    // printk("[DEBUG] *entry = 0x%lx\n", *entry);
}

static inline uint64_t get_pkey(PTE entry)
{
    uint64_t mask = 0x1ful << 54;
    return entry & mask;
}
